...
def initializeStatsDF(del_handler, EXTENDED, stat):
    ...
    for o in del_handler.orgs:
        ...
        for r in del_handler.res_types:
            ...
            # The possible values of each column that is part of the index
            if EXTENDED:
            ...
            # If a specific stat is not provided, all the stats will be computed
            if stat == '':
                cols = [
...
def computation_loop(delegated_subset, a, r, d, o, stats_df, dates_range):
    ...
    if r == 'All':
        ...
        for date in dates_range:
            if date in del_nonZeroDates:
                ...
                if del_sum > 0:
                    stats_df.loc[a, r, d, o, date]['NumOfDelegations'] = del_sum
    
                res_sum = ipv4_del + ipv6_del + asn_res
                if res_sum > 0:
                    stats_df.loc[a, r, d, o, date]['NumOfResources'] = res_sum
                
            # IPCount and IPSpace do not make sense for r = 'All'
            stats_df.loc[a, r, d, o, date]['IPCount'] = np.nan
            stats_df.loc[a, r, d, o, date]['IPSpace'] = np.nan
    else: # r != 'All'
        date_groups =\
                delegated_subset.groupby(delegated_subset['date']\
                                        .map(lambda x: x.strftime('%Y%m%d')))
        ...
        for date in dates_range:
            if r == 'ipv4' or r == 'ipv6':
                ...
                stats_df.loc[a, r, d, o, date]['IPCount'] = float(np.nan)
                stats_df.loc[a, r, d, o, date]['IPSpace'] = float(np.nan)
